...
bool TfliteModelParameters::load(const TfliteModelParametersSchema::Dictionary *fb_dictionary)
{
...
{
    const auto fb_dictionary = TfliteModelParametersSchema::GetDictionary(flatbuffer);
...
{
    auto entry = get(key);
    if (entry != nullptr && entry->type() == TfliteModelParametersSchema::Value::str)
    {
        value = entry->str();
        return true;
    }
...
{
    auto entry = get(key);
    if (entry != nullptr && entry->type() == TfliteModelParametersSchema::Value::bin)
    {
        value = entry->bin();
        return true;
    }
...
{
    auto entry = get(key);
    if (entry != nullptr && entry->type() == TfliteModelParametersSchema::Value::str_list)
    {
...
{
    auto entry = get(key);
    if (entry != nullptr && entry->type() == TfliteModelParametersSchema::Value::int32_list)
    {
...
{
    auto entry = get(key);
    if (entry != nullptr && entry->type() == TfliteModelParametersSchema::Value::float_list)
    {
...
void TfliteModelParameters::print_summary() const
{
    auto print_list = [](const Value& val, void (*printer)(const Value& val))
    {
        MicroPrintf("%s=[", val.key());
        printer(val);
        MicroPrintf("]\n");
    };
    ...
    for(const auto& val : *this)
    {
        ...
        if(dtype == TfliteModelParametersSchema::Value::boolean)
        {
            MicroPrintf("%s=%s\n", key, val->boolean() ? "true" : "false");
        }
        else if(dtype == TfliteModelParametersSchema::Value::i8)
        {
            const auto v = val->i8();
            MicroPrintf("%s=%d\n", key, v);
        }
        else if(dtype == TfliteModelParametersSchema::Value::i16)
        {
            const auto v = val->i16();
            MicroPrintf("%s=%d\n", key, v);
        }
        else if(dtype == TfliteModelParametersSchema::Value::i32)
        {
            const auto v = val->i32();
            MicroPrintf("%s=%d\n", key, v);
        }
        else if(dtype == TfliteModelParametersSchema::Value::i64)
        {
            const auto v = val->i64();
            MicroPrintf("%s=%ld\n", key, v);
        }
        else if(dtype == TfliteModelParametersSchema::Value::u8)
        {
            const auto v = val->u8();
            MicroPrintf("%s=%u\n", key, v);
        }
        else if(dtype == TfliteModelParametersSchema::Value::u16)
        {
            const auto v = val->u16();
            MicroPrintf("%s=%u\n", key, v);
        }
        else if(dtype == TfliteModelParametersSchema::Value::u32)
        {
            const auto v = val->u32();
            MicroPrintf("%s=%u\n", key, v);
        }
        else if(dtype == TfliteModelParametersSchema::Value::u64)
        {
            const auto v = val->u64();
            MicroPrintf("%s=%lu\n", key, v);
        }
        else if(dtype == TfliteModelParametersSchema::Value::f32)
        {
            const auto v = val->f32();
            MicroPrintf("%s=%f\n", key, v);
        }
        else if(dtype == TfliteModelParametersSchema::Value::f64)
        {
            const auto v = val->f64();
            MicroPrintf("%s=%lf\n", key, v);
        }
        else if(dtype == TfliteModelParametersSchema::Value::str)
        {
            const auto v = val->str();
            MicroPrintf("%s=%s\n", key, v);
        }
        else if(dtype == TfliteModelParametersSchema::Value::str_list)
        {
            print_list(*val, [](const Value& val)
            {
        ...
        else if(dtype == TfliteModelParametersSchema::Value::int32_list)
        {
            print_list(*val, [](const Value& val)
            {
        ...
        else if(dtype == TfliteModelParametersSchema::Value::float_list)
        {
            print_list(*val, [](const Value& val)
            {
        ...
        else if(dtype == TfliteModelParametersSchema::Value::bin)
        {
            MicroPrintf("%s=<%d bytes>\n", key, val->bin_length());
        }
    }
}
...
TfliteModelParametersSchema::Value TfliteModelParameters::Value::type(void) const
{
    return value_type();
}
...
{
    return TfliteModelParametersSchema::Entry::key()->c_str();
}
